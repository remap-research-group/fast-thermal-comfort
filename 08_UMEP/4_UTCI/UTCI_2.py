...
base_source_data_dir = f"/storage/project/r-rbasu31-0/shared/{CITY}/01_Input_UTCI/{SCENARIO}"
temp_base_dir = f"/storage/project/r-rbasu31-0/shared/{CITY}/02_IntUTCI/{SCENARIO}"
...
PATHS_CONFIG = {
    "TMRT":     os.path.join(base_source_data_dir, f"Tmrt_2023_179_{TIME}00D.tif"),
    "UROCK":    os.path.join(base_source_data_dir, f"2023_179_{TIME}D_typical_urock_wind_field_WS.tif"),
    "BUILDING": os.path.join(base_source_data_dir, "buildings.tif"),
    "MET":      os.path.join(base_source_data_dir, "metforcing.txt"),
}

# Output directories
final_output_base_dir = f"/storage/project/r-rbasu31-0/shared/{CITY}/03_Output_UTCI/{SCENARIO}"
...
UTCI_FIXED_PARAMS = {
    'TC_TYPE': 1, 'AGE': 35, 'ACTIVITY': 80, 'CLO': 0.9,
    'WEIGHT': 75, 'HEIGHT': 180, 'SEX': 0, 'COMFA': False,
}

_TMRT_BASENAME = os.path.splitext(os.path.basename(PATHS_CONFIG["TMRT"]))[0]
TIME = _TMRT_BASENAME[len("Tmrt_"):] if _TMRT_BASENAME.startswith("Tmrt_") else _TMRT_BASENAME
...
def clear_directory_contents(directory_path: str):
    """
    Remove all children inside a directory while keeping the directory itself.
    Skips NFS .nfs* silly-rename artifacts.
    """
    ...
    for entry_name in os.listdir(directory_path):
        ...
        try:
            ...
            print(f"clear_directory_contents: skipping {entry_path} ({e})")
...
def initialize_worker(worker_profiles_root: str):
    ...
    # Sanity check: Spatial Thermal Comfort must be creatable in this worker.
    try:
        test_alg = registry.createAlgorithmById("umep:Outdoor Thermal Comfort: Spatial Thermal Comfort")
    except Exception as e:
        raise RuntimeError(
            f"UMEP provider loaded but Spatial Thermal Comfort algorithm could not be created: {e}"
        ) from e
    if test_alg is None:
        raise RuntimeError("UMEP provider loaded but Spatial Thermal Comfort algorithm id was not available.")
    del test_alg
...
def set_tile_umep_temp_dir(tile_id: str) -> str:
    """Repoint <plugin_dir>/temp at a fresh empty directory for this tile."""
...
def debuffer_and_save_raster_gdal(
    ...
    original_full_raster_geotransform: tuple,
    original_core_tile_col_off_in_full: int,
    original_core_tile_row_off_in_full: int,
    core_tile_width: int,
    core_tile_height: int,
    ...
    try:
        ...
        if core_data.shape[0] != core_tile_height or core_data.shape[1] != core_tile_width:
            print(f"ERROR: shape mismatch ({core_data.shape}) vs expected ({core_tile_height}, {core_tile_width})")
            return None
        ...
        dst_ds.SetProjection(src_buffered_ds.GetProjection())
        ...
        print(f"ERROR debuffering {buffered_raster_path}: {e}\n{traceback.format_exc()}")
        return None
    finally:
        if src_buffered_ds:
            src_buffered_ds = None
...
def process_tile_with_buffer(
    ...
    original_full_raster_geotransform_tuple, original_full_raster_crs_wkt,
    paths_cfg_dict,
    base_buffered_input_dir_worker,
    base_solweig_out_dir_worker,   # parameter name kept; carries UTCI output dir
    ...
    try:
        ...
        if slice_win_width <= 0 or slice_win_height <= 0:
            msg = f"[{tile_id_str}] Skipped: zero/negative slice window."
            print(msg)
            return msg
        ...
        if all_files_exist:
            ...
            for key in ["TMRT", "UROCK", "BUILDING"]:
                ...
                if tile_specific_buffered_inputs[key] is None:
                    raise RuntimeError(f"Failed to slice {key} for {tile_id_str}")

            # --- 4. Copy MET file (no slicing, just verbatim copy) ---
            for key in ["MET"]:
                in_path = paths_cfg_dict[key]
                out_filename = os.path.basename(in_path)
                out_path = os.path.join(current_tile_buffered_data_dir, out_filename)
            ...
            print(f"[{tile_id_str}] Running Spatial Thermal Comfort. Output: {current_tile_utci_output}")
            tile_temp_dir = set_tile_umep_temp_dir(tile_id_str)
            print(f"[{tile_id_str}] UMEP temp now points at: {tile_temp_dir}")

            _WORKER_PROCESSING.run(
                "umep:Outdoor Thermal Comfort: Spatial Thermal Comfort",
                {
                    'TMRT_MAP':  tile_specific_buffered_inputs.get('TMRT'),
                    'UROCK_MAP': tile_specific_buffered_inputs.get('UROCK'),
                    **UTCI_FIXED_PARAMS,
                    'TC_OUT':    current_tile_utci_output,
                }
            )
            proc_time = (time.time() - tile_processing_start_time) / 60
            print(f"[{tile_id_str}] Spatial Thermal Comfort complete in {proc_time:.2f} mins.")
        ...
        try:
            ...
            print(f"[{tile_id_str}] WARNING: Could not clean up {current_tile_buffered_data_dir}: {e}")

        proc_time = (time.time() - tile_processing_start_time) / 60
        print(f"[{tile_id_str}] Tile task finished in {proc_time:.2f} mins.")

        # --- 6. Debuffer the UTCI Output ---
        ...
        if not all_buffered_utci_files:
            print(f"[{tile_id_str}] WARNING: No UTCI output found in {current_tile_utci_output_dir}.")
        ...
        for buffered_utci_path in all_buffered_utci_files:
            ...
            if debuffered_file:
                ...
                print(f"[{tile_id_str}] ERROR failed to debuffer: {buffered_utci_path}")

        if not debuffered_output_files_for_this_tile and all_buffered_utci_files:
            msg = f"[{tile_id_str}] ERROR: Found UTCI files but failed to debuffer any."
            print(msg)
            return msg

        proc_time = (time.time() - tile_processing_start_time) / 60
        msg = f"[{tile_id_str}] Tile processing complete in {proc_time:.2f} mins. {len(debuffered_output_files_for_this_tile)} UTCI files debuffered."
        print(msg)
        ...
        if isinstance(result_item, str) and ("ERROR" in result_item or "Skipped" in result_item or "WARNING" in result_item):
            ...
            for debuffered_path in result_item:
                if os.path.exists(debuffered_path):
                    ...
                    utci_type_key = parts
                    if utci_type_key not in organized_debuffered_files:
                        organized_debuffered_files[utci_type_key] = []
                    organized_debuffered_files[utci_type_key].append(debuffered_path)
                else:
                    print(f"Warning: missing reported file: {debuffered_path}")
        else:
            failed_tile_processing_count += 1
            print(f"  Unknown result type: {type(result_item)} - {str(result_item)[:200]}")

    print(f"\nTile processing summary: {len(results_from_pool) - failed_tile_processing_count} successful, {failed_tile_processing_count} failed/skipped.")

    if not organized_debuffered_files:
        print("Nothing to merge.")
        elapsed = (time.time() - overall_start_time) / 60
        print(f"\nTotal script execution time: {elapsed:.2f} minutes.")
        return

    for utci_type, file_list_to_merge in organized_debuffered_files.items():
        if not file_list_to_merge:
            continue

        print(f"\nMerging {len(file_list_to_merge)} tiles for: {utci_type}")
        merged_output_path = os.path.join(merged_output_dir, f"{TIME}_{utci_type}_merged_final.tif")
        vrt_path           = os.path.join(merged_output_dir, f"{TIME}_{utci_type}_temp.vrt")
        
        try:
            valid_files_for_vrt = [f for f in file_list_to_merge if os.path.exists(f)]
            if not valid_files_for_vrt:
                print(f"ERROR: no valid files for {utci_type}.")
                continue
            ...
            if vrt_ds is None:
                print(f"ERROR: BuildVRT failed for {utci_type}.")
                continue
            vrt_ds = None
            ...
            if final_ds is None:
                print(f"ERROR: Translate failed for {utci_type}.")
            ...
            print(f"ERROR merging {utci_type}: {e}\n{traceback.format_exc()}")
...
def main():
    ...
    # Use TMRT raster as the spatial reference for tiling
    ...
    original_raster_geotransform = ref_ds.GetGeoTransform()
    original_raster_crs_wkt = ref_ds.GetProjection()
    ref_ds = None
    ...
    requested_processes = min(22, max(1, multiprocessing.cpu_count() - 1), len(tasks_for_pool))
    ...
    if len(results_from_pool) < expected_results:
        ...
        print(f"WARNING: Only collected {len(results_from_pool)}/{expected_results} results. Missing {missing_count}.")
        for worker in workers:

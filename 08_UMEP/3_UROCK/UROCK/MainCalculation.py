...
def main(javaEnvironmentPath,
    ...
    print("Unique temporary directory: ", tmp_dir_unique)
    ...
    feedback.pushInfo(f"H2GIS Database Environment took {time.perf_counter() - t0:.2f}s")
    ...
    feedback.pushInfo(f"Data loading took {time.perf_counter() - t0:.2f}s")

    timeStartCalculation = time.time()
    print(f"SECTION 1 SET H2GIS DATABASE ENVIRONMENT AND LOAD DATA: {time.perf_counter() - section_start}")
    ...
    feedback.pushInfo(f"Create obstacle geometries took {time.perf_counter() - t0:.2f}s")
    print(f"SECTION 2 CREATES OBSTACLE GEOMETRIES: {time.perf_counter() - section_start}")
    ...
    feedback.pushInfo(f"Rotate obstacles took {time.perf_counter() - t0:.2f}s")
    ...
    feedback.pushInfo(f"Calculate obstacle properties took {time.perf_counter() - t0:.2f}s")
    t0 = time.perf_counter()
    ...
    feedback.pushInfo(f"Save rotated obstacles took {time.perf_counter() - t0:.2f}s")
    print(f"SECTION 3 ROTATES OBSTACLES TO THE RIGHT DIRECTION AND CALCULATES GEOMETRY PROPERTIES: {time.perf_counter() - section_start}")
    ...
    feedback.pushInfo(f"Create 2D Rockle zones took {time.perf_counter() - t0:.2f}s")
    print(f"SECTION 4 CREATES THE 2D ROCKLE ZONES: {time.perf_counter() - section_start}")
    ...
    dicOfInitBuildZoneGridPoint, verticalLineTable = benchmark(
    ...
    dicOfVegZoneGridPoint = benchmark(
        InitWindField.affectsPointToVegZone,
        cursor = cursor, 
        gridTable = gridPoint,
        dicOfVegRockleZoneTable = dicOfVegRockleZoneTable,
        prefix = prefix
    ...
    dicOfBuildZoneGridPoint = benchmark(
        InitWindField.removeBuildZonePoints,
        cursor = cursor, 
    ...
    dicOfBuildZoneGridPoint, facadeWithinCavity = benchmark(
    ...
    feedback.pushInfo(f"Set 2D grid took {time.perf_counter() - t0:.2f}s")
    print(f"SECTION 5 SET THE 2D GRID IN THE ROCKLE ZONES: {time.perf_counter() - section_start}")
    ...
    dicOfBuildZone3DWindFactor, maxBuildZoneHeight = benchmark(
        InitWindField.calculates3dBuildWindFactor,
        cursor = cursor,
        dicOfBuildZoneGridPoint = dicOfBuildZoneGridPoint,
        dz = dz,
        prefix = prefix
    ...
    vegetationWeightFactorTable = benchmark(
        InitWindField.calculates3dVegWindFactor,
        cursor = cursor,
    ...
    feedback.pushInfo(f"Initialize 3D wind factors took {time.perf_counter() - t0:.2f}s")
    print(f"SECTION 6 INITIALIZE THE 3D WIND FACTORS IN THE ROCKLE ZONES: {time.perf_counter() - section_start}")
    ...
    allZonesPointFactor = benchmark(
        InitWindField.manageSuperimposition,
        cursor = cursor,
    ...
    feedback.pushInfo(f"Deal with superimposed zones took {time.perf_counter() - t0:.2f}s")
    print(f"SECTION 7 DEALS WITH SUPERIMPOSED ZONES: {time.perf_counter() - section_start}")
    ...
    df_gridBuil = benchmark(
        InitWindField.identifyBuildPoints,
        cursor = cursor,
        gridPoint = gridPoint,
    ...
    df_wind0, nPoints, verticalWindProfile = benchmark(
        InitWindField.setInitialWindField,
        cursor = cursor, 
    ...
    feedback.pushInfo(f"3D wind speed initialization took {time.perf_counter() - t0:.2f}s")
    print(f"SECTION 8 3D WIND SPEED INITIALIZATION: {time.perf_counter() - section_start}")
    ...
    feedback.pushInfo(f"Rasterize data took {time.perf_counter() - t0:.2f}s")
    print(f"SECTION 9 'RASTERIZE' THE DATA - PREPARE MATRICES FOR WIND CALCULATION {time.perf_counter() - section_start}")
    ...
    if not onlyInitialization:
        ...
        u, v, w = \
            WindSolver.solver(  x = x                       , y = y                 , z = z,
                                ...
                                u0 = u0                     , v0 = v0               , w0 = w0, 
                                cursor = cursor, # outputFilePath=outputFilePath,
    ...
    v0[buildingCoordinates[0],buildingCoordinates[1],buildingCoordinates[2]] = 0
    w0[buildingCoordinates[0],buildingCoordinates[1],buildingCoordinates[2]] = 0
    feedback.pushInfo(f"Wind solver application took {time.perf_counter() - t0:.2f}s")
    print(f"SECTION 10 WIND SOLVER APPLICATION: {time.perf_counter() - section_start}")
    ...
    feedback.pushInfo(f"Rotate wind field took {time.perf_counter() - t0:.2f}s")
    print(f"SECTION 11 ROTATE THE WIND FIELD TO THE INITIAL DISPOSITION: {time.perf_counter() - section_start}")
    ...
    rotated_grid = benchmark(
        Obstacles.windRotation,
        cursor=cursor,
    ...
    print(f"{timestamp}: Saving output files from H2")
    print(f"saveRaster={saveRaster}\tsaveVector={saveVector}\tsaveNetcdf={saveNetcdf}")
    start = time.perf_counter()
    if saveRaster and (not saveVector) and (not saveNetcdf):
        print("Streamlining raster saving by not saving vector (save directly from numpy arrays)")
        dicVectorTables = saveData.saveRasterOutputsDirect(
            cursor=cursor,
            gridName=rotated_grid,
            z_out=z_out,
            dz=dz,
            u=u_rot,
            v=v_rot,
        ...
        dicVectorTables, netcdf_path = benchmark(
            saveData.saveBasicOutputs,
            cursor = cursor                , z_out = z_out,
            dz = dz                        , u = u_rot,
            v = v_rot                      , w = w, 
    ...
    print(f"Files saved to scratch directory: {end-start}")

    # Save also the initialisation field if needed
    if debug:
        print("Saving the initialisation field")
    ...
    if not debug:
        timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        print(f"{timestamp}: Closing H2 connection")
        benchmark(
            H2gisConnection.closeAndRemoveH2gisInstance,
            localH2InstanceDir = localH2InstanceDir, 
            conn = conn,
            cur = cursor
        )
        # rmtree(tmp_dir_unique, ignore_errors=True)
    feedback.pushInfo(f"Save outputs took {time.perf_counter() - t0:.2f}s")
    profile.disable()
    print(f"SECTION 12 SAVE EACH OF THE UROCK OUTPUT: {time.perf_counter() - section_start}")
...
def benchmark(func, *args, **kwargs):
    ...
    print(f"{func.__qualname__}() took {elapsed:.6f} seconds")
    return result
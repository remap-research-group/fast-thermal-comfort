...
profiles_path = '/storage/home/hcoda1/2/glin71/qgis_332/'
qgis_prefix_path = "/storage/home/hcoda1/2/glin71/conda_envs/qgis332"
...
base_source_data_dir = "/storage/scratch1/2/glin71/SOLWEIG/input/boston_5k"  # ***CHANGE HERE***
temp_base_dir = "/storage/scratch1/2/glin71/SOLWEIG/SOLWEIG-scratch"
...
# Output directories
final_output_base_dir = "/storage/scratch1/2/glin71/SOLWEIG/output/boston_old_met_data"  # ***CHANGE HERE***
...
PATHS_CONFIG = {
    ...
    "bDSM": os.path.join(base_source_data_dir, "DSM_bldng_hh_reprojected_to_DEM.tif"),
    "cDSM": os.path.join(base_source_data_dir, "cDSM_veg_cleaned_6491_re.tif"),
    "dem": os.path.join(base_source_data_dir, "DEM_merged_1m_6491.tif"),

    # Wall parameters
    "wall_aspect": os.path.join(base_source_data_dir, "Wall_Aspect/wall_aspect_merged.tif"),
    "wall_height": os.path.join(base_source_data_dir, "Wall_Height/wall_height_merged.tif"),

    # Land cover
    "lc": os.path.join(base_source_data_dir, "land_cover_fixed.tif"),

    # Sky view factors directory
    "svf_input_dir": os.path.join(base_source_data_dir, "SVF"),

    # Meteorological forcing file
    "meteo": os.path.join("/storage/scratch1/2/glin71/SOLWEIG/input/Metro_Boston/meteo_hot_typical.txt"),
}
...
SOLWEIG_FIXED_PARAMS = {
    ...
    'POSTURE': 0, 'CYL': True, 'ONLYGLOBAL': True, 'UTC': -4,
...
def prepare_worker_umep_plugin(worker_profile_dir: str) -> tuple[str, str, str]:
    """
    Create a worker-private QGIS profile plugin directory and copy the UMEP plugin into it
    so that processing_umep/temp is private to that worker.
    ...
    worker_umep_temp_dir = os.path.join(worker_umep_plugin_dir, "temp")
    ...
    clear_directory_contents(worker_umep_temp_dir)
    return worker_plugins_dir, worker_umep_plugin_dir, worker_umep_temp_dir
...
def initialize_worker(worker_profiles_root: str):
    ...
    """
    global _WORKER_PROCESSING, _WORKER_QGS_APP, _WORKER_TEMP_DIR, _WORKER_PLUGIN_DIR, _WORKER_PROFILE_DIR, _WORKER_UMEP_PROVIDER
    ...
    worker_plugins_dir, worker_umep_plugin_dir, worker_umep_temp_dir = prepare_worker_umep_plugin(worker_profile_dir)
    ...
    # These are helpful for QGIS settings isolation, even though the key collision problem
    # is the plugin's temp folder itself.
    os.environ["QGIS_CUSTOM_CONFIG_PATH"] = worker_custom_config_root
    os.environ["QGIS_PROFILE"] = worker_profile_name
    os.environ["TMPDIR"] = worker_umep_temp_dir
    os.environ["TEMP"] = worker_umep_temp_dir
    os.environ["TMP"] = worker_umep_temp_dir
    ...
    _WORKER_TEMP_DIR = worker_umep_temp_dir
    ...
    print(f"[worker pid={worker_pid}] Private UMEP temp dir: {_WORKER_TEMP_DIR}")



def clear_worker_umep_temp_dir():
    """
    Clears the current worker's private UMEP temp directory so stale intermediate SVF TIFFs do not
    interfere with the next SOLWEIG run.
    """
    if _WORKER_TEMP_DIR is None:
        raise RuntimeError("Worker UMEP temp directory has not been initialized.")
    clear_directory_contents(_WORKER_TEMP_DIR)


def shutdown_worker_qgis():
    """Shut down the worker-local QGIS app cleanly."""
    global _WORKER_QGS_APP, _WORKER_PROCESSING, _WORKER_TEMP_DIR, _WORKER_PLUGIN_DIR, _WORKER_PROFILE_DIR, _WORKER_UMEP_PROVIDER

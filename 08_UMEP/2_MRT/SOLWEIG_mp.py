...
def clear_directory_contents(directory_path: str):
    ...
    On NFS, deleting a file that is still held open by some process triggers a
    silly-rename to ".nfsXXXXXXXX". A subsequent unlink of that silly-renamed
    file while it is still open raises OSError(EBUSY). We skip ".nfs*" entries
    outright and swallow per-entry OSErrors so a single stuck handle does not
    abort the whole tile run. NFS reaps the silly-renames once the last open
    handle is closed, and SOLWEIG's expected filenames (svf.tif, svfN.tif, ...)
    will not collide with the random ".nfs*" names anyway.
    """
    ...
    for entry_name in os.listdir(directory_path):
        # NFS silly-rename artifacts — let NFS clean them up when the holder closes.
        ...
        try:
            ...
            # EBUSY (16) on NFS means the file is still open elsewhere; harmless.
